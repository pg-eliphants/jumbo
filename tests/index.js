...
t("Empty array", async () => [
    true,
    Array.isArray((await sql`select ${sql.array([], 1009)} as x`)[0].x),
]);

t("String array", async () => [
    "123",
    (await sql`select ${"{1,2,3}"}::int[] as x`)[0].x.join(""),
]);

t("Array of Integer", async () => [
    "3",
    (await sql`select ${sql.array([1, 2, 3])} as x`)[0].x[2],
]);

t("Array of String", async () => [
    "c",
    (await sql`select ${sql.array(["a", "b", "c"])} as x`)[0].x[2],
]);
...
t("Array of Date", async () => {
    ...
    return [
        now.getTime(),
        (
            await sql`select ${sql.array([now, now, now])} as x`
        )[0].x[2].getTime(),
    ];
});
...
t("Nested array n3", async () => [
    "6",
    (await sql`select ${sql.array([[[1, 2]], [[3, 4]], [[5, 6]]])} as x`)[0]
        .x[2][0][1],
]);
...
t("Escape in arrays", async () => [
    'Hello "you",c:\\windows',
    (
        await sql`select ${sql.array(['Hello "you"', "c:\\windows"])} as x`
    )[0].x.join(","),
]);
...
t("Escapes", async () => {
    return [
        'hej"hej',
        Object.keys((await sql`select 1 as ${sql('hej"hej')}`)[0])[0],
    ];
});
...
t("null for int", async () => {
    ...
    return [
        1,
        (await sql`insert into test values(${null})`).count,
        await sql`drop table test`,
    ];
});
...
t("Transaction rolls back", async () => {
    ...
    return [
        0,
        (await sql`select a from test`).count,
        await sql`drop table test`,
    ];
});
...
t("Transaction succeeds on caught savepoint", async () => {
    ...
    return [
        "2",
        (await sql`select count(1) from test`)[0].count,
        await sql`drop table test`,
    ];
});
...
t("Many transactions at beginning of connection", async () => {
    ...
    const xs = await Promise.all(
        Array.from({ length: 100 }, () => sql.begin((sql) => sql`select 1`)),
    );
...
t("Helpers in Transaction", async () => {
    return [
        "1",
        (await sql.begin(async (sql) => await sql`select ${sql({ x: 1 })}`))[0]
            .x,
    ];
});
...
t("Transform undefined in array", async () => {
    ...
    return [
        null,
        (
            await sql`select * from (values ${sql([
                undefined,
                undefined,
            ])}) as x(x, y)`
        )[0].y,
...
t("Null sets to null", async () => [
    null,
    (await sql`select ${null} as x`)[0].x,
]);

t("Throw syntax error", async () => [
    "42601",
    (await sql`wat 1`.catch((x) => x)).code,
]);
...
t("Login without password", async () => {
    return [
        true,
        (await postgres({ ...options, ...login })`select true as x`)[0].x,
    ];
});
...
t("Login using MD5", async () => {
    return [
        true,
        (await postgres({ ...options, ...login_md5 })`select true as x`)[0].x,
    ];
});
...
t("Login using scram-sha-256", async () => {
    return [
        true,
        (await postgres({ ...options, ...login_scram })`select true as x`)[0].x,
    ];
});
...
t("Point type", async () => {
    ...
    return [
        20,
        (await sql`select x from test`)[0].x[1],
        await sql`drop table test`,
    ];
});
...
t("Point type array", async () => {
    ...
    await sql`create table test (x point[])`;
    await sql`insert into test (x) values (${sql.array([
        sql.types.point([10, 20]),
        sql.types.point([20, 30]),
    ])})`;
    return [
        30,
        (await sql`select x from test`)[0].x[1][1],
        await sql`drop table test`,
    ];
});
...
t("sql file throws", async () => [
    "ENOENT",
    await sql.file(rel("selectomondo.sql")).catch((x) => x.code),
]);
...
t("transform column", async () => {
    ...
    return [
        "dlrow_olleh",
        Object.keys((await sql`select * from test`)[0])[0],
        await sql`drop table test`,
    ];
});
...
t("Transform nested json in arrays", async () => {
    ...
    return [
        "aBcD",
        (await sql`select '[{"a_b":1},{"c_d":2}]'::jsonb as x`)[0].x
            .map(Object.keys)
            .join(""),
    ];
});
    ...
    `
            ...
            .map((x) => {
                ...
                for (const key in x)
                    result = [
                        ...Object.keys(x[key]),
                        ...Object.keys(x[key].deeplyNestedObj),
                    ];
                return result;
            })[0]
            .join("_"),
    ...
    `
            ...
            .map((x) => {
                ...
                for (const key in x)
                    result = [
                        ...Object.keys(x[key][0]),
                        ...Object.keys(x[key][0].deeplyNestedArray[0]),
                    ];
                return result;
            })[0]
            .join("_"),
...
t("unsafe", async () => {
    ...
    return [
        1,
        (await sql.unsafe("insert into test values ($1) returning *", [1]))[0]
            .x,
        await sql`drop table test`,
    ];
});
...
t("simple query using unsafe with multiple statements", async () => {
    return [
        "1,2",
        (await sql.unsafe("select 1 as x;select 2 as x"))
            .map((x) => x[0].x)
            .join(),
    ];
});
...
t("simple query using simple() with multiple statements", async () => {
    return [
        "1,2",
        (await sql`select 1 as x;select 2 as x`.simple())
            .map((x) => x[0].x)
            .join(),
    ];
});
...
t(
    "listen result reports correct connection state after reconnection",
    async () => {
        const sql = postgres(options),
            xs = [];
...
t("has server parameters", async () => {
    return [
        "postgres.js",
        await sql`select 1`.then(() => sql.parameters.application_name),
    ];
});
...
t("let postgres do implicit cast of unknown types", async () => {
    ...
    const [{ x }] =
        await sql`insert into test values (${new Date().toISOString()}) returning *`;
    return [true, x instanceof Date, await sql`drop table test`];
});

t("only allows one statement", async () => [
    "42601",
    await sql`select 1; select 2`.catch((e) => e.code),
]);
...
t("little bobby tables", async () => {
    ...
    return [
        name,
        (await sql`select name from students`)[0].name,
...
t("dynamic table name", async () => {
    ...
    return [
        0,
        (await sql`select * from ${sql("test")}`).count,
        await sql`drop table test`,
    ];
});
...
t("dynamic schema name", async () => {
    ...
    return [
        0,
        (await sql`select * from ${sql("public")}.test`).count,
        await sql`drop table test`,
    ];
});
...
t("dynamic column name", async () => {
    return [
        "!not_valid",
        Object.keys((await sql`select 1 as ${sql("!not_valid")}`)[0])[0],
    ];
});
...
t("dynamic insert", async () => {
    ...
    return [
        "the answer",
        (await sql`insert into test ${sql(x)} returning *`)[0].b,
        await sql`drop table test`,
    ];
});
...
t("dynamic insert pluck", async () => {
    ...
    return [
        null,
        (await sql`insert into test ${sql(x, "a")} returning *`)[0].b,
        await sql`drop table test`,
    ];
});
...
t("array insert", async () => {
    ...
    return [
        2,
        (
            await sql`insert into test (a, b) values ${sql([1, 2])} returning *`
        )[0].b,
        await sql`drop table test`,
    ];
});
...
t("dynamic multi row insert", async () => {
    ...
    return [
        "the answer",
        (await sql`insert into test ${sql([x, x])} returning *`)[1].b,
        await sql`drop table test`,
    ];
});
...
t("dynamic select array", async () => {
    ...
    return [
        "yay",
        (await sql`select ${sql(["a", "b"])} from test`)[0].b,
        await sql`drop table test`,
    ];
});
...
t("dynamic returning array", async () => {
    ...
    return [
        "yay",
        (
            await sql`insert into test (a, b) values (42, 'yay') returning ${sql(
                ["a", "b"],
            )}`
        )[0].b,
        await sql`drop table test`,
...
t("Unsafe cursor", async () => {
    const order = [];
    await sql
        .unsafe("select 1 as x union select 2 as x")
        .cursor(async ([x]) => {
            order.push(x.x + "a");
            await delay(100);
            order.push(x.x + "b");
...
t("Cursor cancel", async () => {
    let result;
    await sql`select * from generate_series(1,10) as x`.cursor(
        async ([{ x }]) => {
            result = x;
            return sql.CLOSE;
        },
...
t("Multiple Cursors", { timeout: 2 }, async () => {
    const result = [];
    await sql.begin(async (sql) => [
        await sql`select 1 as cursor, x from generate_series(1,4) as x`.cursor(
            async ([row]) => {
                result.push(row.x);
                await new Promise((r) => setTimeout(r, 20));
            },
        ),
        await sql`select 2 as cursor, x from generate_series(101,104) as x`.cursor(
            async ([row]) => {
                result.push(row.x);
...
t(
    "Async Iterator Cursor custom with less results than batch size",
    async () => {
        const order = [];
        for await (const x of sql`select * from generate_series(1,20)`.cursor(
            21,
        ))
            order.push(x.length);
        return ["20", order.join(",")];
...
t("Big result", async () => {
    return [
        100000,
        (await sql`select * from generate_series(1, 100000)`).count,
    ];
});
...
t(
    "Query and parameters on errorare not enumerable if debug is not set",
    async () => {
        const sql = postgres({ ...options, debug: false });

        return [
            false,
            await sql`selec ${1}`.catch(
                (err) =>
                    err.propertyIsEnumerable("parameters") ||
                    err.propertyIsEnumerable("query"),
            ),
        ];
    },
);
...
t(
    "connect_timeout error message includes host:port",
    { timeout: 20 },
    async () => {
        const connect_timeout = 0.2;
        const server = net.createServer();
...
t("Result has columns spec", async () => [
    "x",
    (await sql`select 1 as x`).columns[0].name,
]);
...
t("Insert empty array", async () => {
    ...
    return [
        Array.isArray(
            (
                await sql`insert into tester (ints) values (${sql.array(
                    [],
                )}) returning *`
            )[0].ints,
        ),
...
t("Insert array in sql()", async () => {
    ...
    return [
        Array.isArray(
            (
                await sql`insert into tester ${sql({
                    ints: sql.array([]),
                })} returning *`
            )[0].ints,
        ),
...
t('prepares unsafe query when "prepare" option is true', async () => {
    ...
    const result = await sql.unsafe(
        "select * from pg_prepared_statements where name <> $1",
        ["bla"],
        { prepare: true },
    );
    return [true, result.some((x) => (x.name = result.statement.name))];
...
t(
    "Recreate prepared statements on transformAssignedExpr error",
    { timeout: 1 },
    async () => {
        const insert = () =>
            sql`insert into test (name) values (${"1"}) returning name`;
...
t("Throws correct error when retrying in transactions", async () => {
    ...
    const error = await sql
        .begin((sql) => sql`insert into test (x) values (${false})`)
        .catch((e) => e);
...
t(
    ...
    async () => {
        const s1 = postgres({ idle_timeout }),
            ...
            sql = postgres("postgres://localhost:5432,localhost:5433", {
                idle_timeout,
                max: 1,
            }),
            result = [];
        ...
        const id1 = (
            await s1`select system_identifier as x from pg_control_system()`
        )[0].x;
        const id2 = (
            await s2`select system_identifier as x from pg_control_system()`
        )[0].x;

        const x1 = await sql`select 1`;
        result.push(
            (
                await sql`select system_identifier as x from pg_control_system()`
            )[0].x,
        );
...
t("Copy write as first", async () => {
    ...
    const writable =
        await first`COPY test FROM STDIN WITH(FORMAT csv, HEADER false, DELIMITER ',')`.writable();
    writable.write("1\n");
    writable.write("1\n");
    writable.end();
...
t("Cancel queued query", async () => {
    ...
    const tx = sql.begin(
        (sql) => (query.cancel(), sql`select pg_sleep(0.5) as hej, 'hejsa'`),
    );
    const error = await query.catch((x) => x);
    await tx;
    return ["57014", error.code];
...
t("Describe a statement", async () => {
    ...
    const r =
        await sql`select name, age from tester where name like $1 and age > $2`.describe();
    return [
        "25,23/name:25,age:23",
        `${r.types.join(",")}/${r.columns
            .map((c) => `${c.name}:${c.type}`)
            .join(",")}`,
        await sql`drop table tester`,
    ];
...
t("Include table oid and column number in column details", async () => {
    ...
    const r =
        await sql`select name, age from tester where name like $1 and age > $2`.describe();
    const [{ oid }] =
        await sql`select oid from pg_class where relname = 'tester'`;

    return [
        `table:${oid},number:1|table:${oid},number:2`,
        `${r.columns
            .map((c) => `table:${c.table},number:${c.number}`)
            .join("|")}`,
        await sql`drop table tester`,
    ];
});
...
t("Describe a statement without columns", async () => {
    await sql`create table tester (name text, age int)`;
    const r =
        await sql`insert into tester (name, age) values ($1, $2)`.describe();
    return [
        "2,0",
        `${r.types.length},${r.columns.length}`,
        await sql`drop table tester`,
...
t("Large object", async () => {
    ...
    await new Promise((r) =>
        lo
            .readable()
            .then((x) => x.on("data", (x) => out.update(x)).on("end", r)),
    );
...
t("Catches type serialize errors in transactions", async () => {
    ...
    return [
        "watSerialize",
        await sql
            .begin((sql) => (sql`select 1`, sql`select ${"wat"}`))
            .catch((e) => e.message),
    ];
});
...
t(
    "Ensure reconnect after max_lifetime with transactions",
    { timeout: 5 },
    async () => {
        const sql = postgres({
            max_lifetime: 0.01,
...
t("Supports multiple nested fragments with parameters", async () => {
    const [{ b }] =
        await sql`select * ${sql`from ${sql`(values (2, ${1}::int)) as x(${sql([
            "a",
            "b",
        ])})`}`}`;
    return [1, b];
...
t("Insert array with undefined throws", async () => {
    ...
    return [
        "UNDEFINED_VALUE",
        await sql`insert into test ${sql({ x: [1, undefined, 3] })}`.catch(
            (e) => e.code,
        ),
        await sql`drop table test`,
    ];
...
t("concurrent cursors", async () => {
    ...
    await Promise.all(
        [...Array(7)]
            .map((x, i) => [
                sql`select ${i}::int as a, generate_series(1, 2) as x`.cursor(
                    ([x]) => xs.push(x.a + x.x),
                ),
            ])
            .flat(),
