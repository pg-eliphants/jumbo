...
*/
function Postgres(unParsedURL, _options) {
    const options = parseOptions(unParsedURL, _options),
        //"" no_subscribe" is undocumented
        /*
             ...options,
        no_subscribe: true,
    ...
    async function reserve() {
        ...
        move(c, reserved);
        c.reserved = () =>
            q.length ? c.execute(q.shift()) : move(c, reserved);
        ...
        function handler(q) {
            c.queue === full ? q.push(q) : c.execute(q) || move(c, full);
        }
    }
